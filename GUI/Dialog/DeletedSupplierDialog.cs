    ...
    public partial class DeletedSupplierDialog : BaseForm
    {
        ...
        private void ShowDeletedSupplier(List<SupplierDTO> list)
        {
            try
            {
                ...
                BeginInvoke(new Action(() => { dgvDeletedSupplier.ClearSelection(); }));
                for(int i = 0; i < list.Count; i++) {
                    dgvDeletedSupplier.Rows.Add(i + 1, list[i].id, list[i].name, list[i].phone, list[i].email, list[i].note, list[i].status, list[i].created, list[i].updated, list[i].deleted);
                }
            } catch(Exception ex)
            {
